...
void panel()
{
    ...
    cursor_to_pos(0, get_window_cols() / 4);
    cout << "-------------------------------------------------" << endl;
    reset_color();
    change_color_rgb(51, 246, 255);
    cursor_to_pos(2, get_window_cols() / 4);
    cout << "Welcome to 2 player snake game ! :)" << endl;
    cursor_to_pos(3, get_window_cols() / 4);
    cout << "The control keys for the second player are arrow keys " << endl;
    cursor_to_pos(4, get_window_cols() / 4);
    cout << "Please Enter your names and select the game mode :') " << endl;
    cursor_to_pos(5, get_window_cols() / 4);
    cout << "At the start of the game if you hit yourself it wont make you lose until one apple is eaten" << endl;
    cursor_to_pos(6, get_window_cols() / 4);
    cout << "The game will only work when 2 players are playing" << endl;
    reset_color();
    delay(9500);
    clear_screen();
    change_color_rgb(5, 49, 249);
    cursor_to_pos(0, get_window_cols() / 4);
    ...
    cursor_to_pos(5, get_window_cols() / 4);
    cout << "4.The challenge(time limit)" << endl;
    cursor_to_pos(6, get_window_cols() / 4);
    cout << "5.Exit"
         << " ";
    cin >> Gamemode;
...
int main()
{
    ...
    case 1:
        while (!gameover)
        {
            ...
            delay(240); // it allows fps for slower speed increase the number and faster speed decrease the number
            hidecursor();
            // timer++;
        }
        break;
    ...
    case 4:
        while (!gameover)
        {
            ...
            delay(220); // it allows fps for slower speed increase the number and faster speed decrease the number
            hidecursor();
            // timer++;
        }
        break;
